...
resourcestring
  ...
  rsTarget = 'Target';
  rsMhS = 'Mh/s';
  rsMinerPayload = 'Miner Payload';
  rsProofOfWork = 'Proof of Work';
  rsSafeBoxHash = 'SafeBox Hash';
  rsProtocol = 'Protocol';
  rsAccWork = 'Acc.Work';
  rsDeviation = 'Deviation';
  rsTimeAverage = 'Time average';
  rsNoData = '(no data)';

{ TAccountsGrid }

Const CT_ColumnHeader : Array[TAccountColumnType] Of String =
  (rsAccountN, rsKey, rsBalance, rsUpdated, rsNOper, rsState, rsName, rsType);
  ...
  if (ARow=0) then begin
    ...
    if (ARow<=FOperationsResume.Count) then begin
      ...
      if ACol=0 then begin
        if opr.time=0 then s := rsPending
