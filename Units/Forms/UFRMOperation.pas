...
resourcestring
  rsErrorEncodin = 'Error encoding payload of sender account %s: %s';
  rsSenderAccoun = 'Sender account private key not found in Wallet';
  rsChangePrivat = 'Change private key to %s';
  rsNoOperationS = 'No operation selected';
  rsNoValidOpera = 'No valid operation to execute';
  rsTotalAmountT = 'Total amount that dest will receive: %s%s';
  rsExecuteOpera = 'Execute %s operations?%sOperation: %s%s%sTotal fee: %s%s%'
    +'sNote: This operation will be transmitted to the network!';
  rsExecuteThisO = 'Execute this operation:%s%s%s%s%sNote: This operation will'
    +' be transmitted to the network!';
  rsSuccessfully = 'Successfully executed %s operations!%s%s%s';
  rsOneOrMoreOfY = 'One or more of your operations has not been executed:%'
    +'sErrors:%s%s%s%sTotal successfully executed operations: %s';
  rsWalletPasswo = 'Wallet password';
  rsEnterWalletP = 'Enter wallet password';
  rsALLBALANCE = 'ALL BALANCE';
  rsAccounts = 'Accounts';
  rsAccounts2 = '%s accounts';
  rsAccount = 'Account';
  rsInvalidFeeVa = 'Invalid fee value "%s"';
  rsCannotBuyAcc = 'Cannot buy accounts with multioperations. Use only 1 '
    +'account';
  rsInvalidAccou = 'Invalid account to buy %s';
  rsInvalidAccou2 = 'Invalid account number';
  rsAccountIsNot = 'Account %s is not for sale';
  rsInvalidAmoun = 'Invalid amount value';
  rsAccountPrice = 'Account price %s';
  rsInsufficient = 'Insufficient funds';
  rsMustSelectAN = 'Must select a new private key';
  rsInvalidSelec = 'Invalid selected key';
  rsThisOperatio = 'This operation needs PROTOCOL 2 active';
  rsCannotChange =
    'Cannot change info with multioperations. Use only 1 account';
  rsAccountIsLoc = 'Account %s is locked until block %s';
  rsInvalidSigne = 'Invalid signer account';
  rsSignerAccoun = 'Signer account does not exists %s';
  rsSignerAccoun2 = 'Signer account %s is locked until block %s';
  rsSignerAccoun3 = 'Signer account %s is not ower of account %s';
  rsIsNotAValidN = '"%s" is not a valid name: %s';
  rsNameIsUsedBy = 'Name "%s" is used by account %s';
  rsInvalidType = 'Invalid type %s';
  rsAccountNameA = 'Account name and type are the same. Not changed';
  rsCanBeUpdated = '%s can be updated';
  rsNewKeyType = 'New key type: %s';
  rsSelectAChang = 'Select a change type';
  rsNewPublicKey = 'New public key is the same public key';
  rsSignerAccoun4 = 'Signer account %s is not ower of delisted account %s';
  rsNoWalletKeys = 'No wallet keys';
  rsNoSenderAcco = 'No sender account';
  rsPrivateKeyOf = 'Private key of account %s not found in wallet';
  rsWalletIsPass = 'Wallet is password protected. Need password';
  rsOnlyPublicKe = 'Only public key of account %s found in wallet. You cannot '
    +'operate with this account';
  rsMustSelectAn2 = 'Must select an operation';
  rsEncryptedWit = 'Encrypted with sender public key';
  rsEncryptedWit2 = 'Encrypted with destination public key';
  rsEncryptedWit3 = 'Encrypted with old public key';
  rsEncryptedWit4 = 'Encrypted with new public key';
  rsEncryptedWit5 = 'Encrypted with target public key';
  rsEncryptedWit6 = 'Encrypted with signer public key';
  rsEncryptedWit7 = 'Encrypted with buyer public key';
  rsInvalidPrice = 'Invalid price';
  rsInvalidSelle = 'Invalid seller account';
  rsSellerAccoun = 'Seller account does not exists %s';
  rsPublicKey = 'Public key: %s';
  rsNewPublicKey2 = 'New public key for private sale is the same public key';
  rsInsertLockin = 'Insert locking block';
  rsSelectASaleT = 'Select a sale type';
  rsInvalidDestA = 'Invalid dest. account (%s)';
  rsInvalidAmoun2 = 'Invalid amount (%s)';
  rsSenderAndDes = 'Sender and dest account are the same';
  rsErrorEncrypt = 'Error encrypting';
  rsInvalidDestA2 = 'Invalid dest account number';
  rsInvalidSigne2 = 'Invalid signer account number';
  rsInvalidAccou3 = 'Invalid account to buy number';
  rsInvalidPaylo = 'Invalid payload encrypted account number: %s';
  rsMustSelectAP = 'Must select a private key';
  rsMustSelectCh = 'Must select change type';
  rsSelectedPriv = 'Selected private key is not valid to encode';
  rsThisOperatio2 = 'This operation does not allow this kind of payload';
  rsMustSelectAn3 = 'Must select an encryption option for payload';
  rsPayloadSizeI = 'Payload size is bigger than %s (%s)';
...
loop_start:
      ...
      if i<0 then begin
        raise Exception.Create(rsSenderAccoun);
      end;
    ...
    if (Length(_senderAccounts)>1) then begin
      ...
      if Application.MessageBox(PChar(Format(rsExecuteThisO, [#10, #10,
        operation_to_string, #10, #10])),
        PChar(Application.Title),MB_YESNO+MB_ICONINFORMATION+MB_DEFBUTTON2)<>IdYes then exit;
    end;
    i := FNode.AddOperations(nil,ops,Nil,errors);
    if (i=ops.OperationsCount) then begin
      Application.MessageBox(PChar(Format(rsSuccessfully, [inttostr(i), #10,
        #10, operation_to_string])), PChar(Application.Title), MB_OK+
        MB_ICONINFORMATION);
      ModalResult := MrOk;
    end else if (i>0) then begin
      Application.MessageBox(PChar(Format(rsOneOrMoreOfY, [#10, #10, errors,
        #10, #10, inttostr(i)])), PChar(Application.Title), MB_OK+MB_ICONWARNING
        );
      ModalResult := MrOk;
    end else begin
...
begin
  if FWalletKeys.IsValidPassword then begin
    ...
    s := '';
    Repeat
      if not InputQuery(rsWalletPasswo, rsEnterWalletP, s) then exit;
...
begin
  ...
  FDisabled := true;
  Try
    lblAccountCaption.Caption := rsAccount;
    lblAccountsCount.Visible := false;
    lblAccountsCount.caption := Format(rsAccounts2, [inttostr(
      senderAccounts.Count)]);
    balance := 0;
    if SenderAccounts.Count<=0 then begin
      ebSenderAccount.Text := '';
      ...
      lblAccountCaption.Caption := rsAccounts;
...
begin
  ...
  Try
    ...
    if (amount+DefaultFee > SenderAccount.balance) then begin
      errors := rsInsufficient;
      exit;
    end;
    if cbBuyNewKey.ItemIndex<0 then begin
      errors := rsMustSelectAN;
      exit;
    end;
...
begin
  ...
  try
    ...
    if (errCode>0) then begin
      errors := Format(rsInvalidType, [ebChangeType.text]);
      Exit;
    end;
...
begin
  ...
  try
    if rbChangeKeyWithAnother.Checked then begin
      ...
      if (i<0) Or (i>=WalletKeys.Count) then raise Exception.Create(
        rsInvalidSelec);
      NewPublicKey := WalletKeys.Key[i].AccountKey;
...
begin
  ...
  try
    ...
    if Not Assigned(WalletKeys) then begin
      errors := rsNoWalletKeys;
      lblGlobalErrors.Caption := errors;
      exit;
    end;
    if SenderAccounts.Count=0 then begin
      errors := rsNoSenderAcco;
      lblGlobalErrors.Caption := errors;
      exit;
    end else begin
  ...
  if (PageControlOpType.ActivePage = tsTransaction) then begin
    ...
    errors := rsMustSelectAn2;
  end;
  if (PageControlOpType.ActivePage=tsTransaction) then begin
    rbEncryptedWithOldEC.Caption := rsEncryptedWit;
    rbEncryptedWithEC.Caption := rsEncryptedWit2;
  end else if (PageControlOpType.ActivePage=tsChangePrivateKey) then begin
    rbEncryptedWithOldEC.Caption := rsEncryptedWit3;
    rbEncryptedWithEC.Caption := rsEncryptedWit4;
  end else if ((PageControlOpType.ActivePage=tsListForSale) Or (PageControlOpType.ActivePage=tsDelist)) then begin
    rbEncryptedWithOldEC.Caption := rsEncryptedWit5;
    rbEncryptedWithEC.Caption := rsEncryptedWit6;
  end else if (PageControlOpType.ActivePage=tsBuyAccount) then begin
    ...
    if (rbEncryptedWithOldEC.Checked) then begin
      ...
      if (PageControlOpType.ActivePage=tsTransaction) or (PageControlOpType.ActivePage=tsListForSale) or (PageControlOpType.ActivePage=tsDelist)
        ...
        if public_key.EC_OpenSSL_NID<>CT_Account_NUL.accountInfo.accountKey.EC_OpenSSL_NID then begin
          ...
          valid := false;
          errors := rsSelectedPriv;
          exit;
        end;
      end else begin
        errors := rsThisOperatio2;
      end;
      ...
      errors := rsMustSelectAn3;
    end;
  finally
    if valid then begin
      if length(payload_encrypted)>CT_MaxPayloadSize then begin
        valid := false;
        errors := Format(rsPayloadSizeI, [inttostr(CT_MaxPayloadSize), Inttostr(
          length(payload_encrypted))]);
      end;
    end;
    if valid then begin
