...
begin
  ...
  if Assigned(wk.PrivateKey) then begin
    if InputQuery(rsExportPrivat, rsInsertAPassw, pwd1)
      then begin
      if InputQuery(rsExportPrivat, rsRepeatThePas, pwd2)
        then begin
...
begin
  ...
  try
    ...
    While (Not wki.IsValidPassword) do begin
      if not InputQuery(rsImport, rsEnterTheWall, pwd) then exit;
 ...
 parseResult : Boolean;
  ...
  begin
      Repeat
        s := '';
        desenc := '';
        if InputQuery(rsImportPrivat, rsEnterThePass, s) then begin
...
begin
  if FWalletKeys.IsValidPassword then begin
    s := ''; s2 := '';
    if not InputQuery(rsChangePasswo, rsEnterNewPass, s) then exit;
    if trim(s)<>s then raise Exception.Create(rsPasswordCann);
    if not InputQuery(rsChangePasswo, rsEnterNewPass2, s2) then exit;
