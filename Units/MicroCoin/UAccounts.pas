...
Const CT_MicroCoin_Base64_Charset : ShortString = 'abcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*()-+{}[]\_:"|<>,.?/~';
...
begin
  Result := False; errors := '';
  if (length(new_name)<CT_MicroCoin_name_min_length) Or (length(new_name)>CT_MicroCoin_name_max_length) then begin
  ...
  for i:=1 to length(new_name) do begin
    ...
    if (i=1) then begin
      // First char can't start with a number
      While (j<=length(CT_MicroCoin_FirstChar_Charset)) and (new_name[i]<>CT_MicroCoin_FirstChar_Charset[j]) do inc(j);
      if j>length(CT_MicroCoin_FirstChar_Charset) then begin
        errors := 'Invalid char '+new_name[i]+' at first pos';
        Exit; // Not found
      end;
    end else begin
      While (j<=length(CT_MicroCoin_Base64_Charset)) and (new_name[i]<>CT_MicroCoin_Base64_Charset[j]) do inc(j);
      if j>length(CT_MicroCoin_Base64_Charset) then begin
...
begin
  ...
  // reward
  if (newOperationBlock.reward<>TMicroCoinProtocol.GetRewardForNewLine(newOperationBlock.block)) then begin
  ...
  if (newOperationBlock.block > 0) then begin
    ...
    if (newOperationBlock.protocol_version<>CurrentProtocol) then begin
      ...
      If (lastBlock.protocol_version>newOperationBlock.protocol_version) then begin
        errors := 'Invalid MicroCoin protocol version: '+IntToStr( newOperationBlock.protocol_version )+' Current: '+IntToStr(CurrentProtocol)+' Previous:'+IntToStr(lastBlock.protocol_version);
        exit;
      end;
  ...
  if (newOperationBlock.compact_target <> TMicroCoinProtocol.TargetToCompact(target_hash)) then begin
