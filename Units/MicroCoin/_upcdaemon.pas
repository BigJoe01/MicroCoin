...
Type
  ...
  TPCDaemonThread = Class(TTCPhread)
  private
    FIniFile : TIniFile;
  protected
...
begin
  ...
  FIniFile := TIniFile.Create(ExtractFileDir(Application.ExeName)+PathDelim+'microcoin_daemon.ini');
  If FIniFile.ReadBool(CT_INI_SECTION_GLOBAL,CT_INI_IDENT_SAVELOGS,true) then begin
    _FLog.SaveTypes:=CT_TLogTypes_ALL;
    _FLog.FileName:=TFolderHelper.GetMicroCoinDataFolder+PathDelim+'microcoin_'+FormatDateTime('yyyymmddhhnn',Now)+'.log';
...
procedure TPCDaemonMapper.OnMicroCoinInThreadLog(logtype: TLogType;
...
begin
  ...
  D.DisplayName:='Micro Coin Daemon';
  D.Name:='MicroCoinDaemon';
